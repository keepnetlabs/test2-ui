  ...
  >
    <template #selection-all-slot v-if="hasSelectionSlot">
      ...
      <v-tooltip bottom opacity="1">
        <template v-slot:activator="{ on }">
          <span v-on="on">
            <v-btn
              class="btn-selected-hover mr-1"
              icon
              :style="getRemoveUsersButtonStyle"
        ...
        <span class="tooltip-span">{{ removeUsersTooltip }}</span>
      </v-tooltip>
    </template>
    <template v-if="hasRowActions" #datatable-row-actions="{ scope }">
      ...
      <RowActionsMenu>
        ...
        <TargetUserRowActionsRemoveFromGroupButton
          ...
          :scope="scope"
          :isGroupEditable="isGroupEditableParam"
...
export default {
  ...
  computed: {
    ...
    isGroupEditableParam() {
      const value = this.$route?.params?.isGroupEditable;
      if (value === false || value === "false") return false;
      if (value === true || value === "true") return true;
      return !this.isScimGroup && !this.isGoogleGroup;
    },
    canRemoveUsers() {
      return this.isGroupEditableParam;
    },
    getRemoveUsersButtonStyle() {
      if (this.canRemoveUsers) return null;
      return {
        opacity: 0.5,
        pointerEvents: "none"
      };
    ...
    removeUsersTooltip() {
      if (this.canRemoveUsers) return "Remove Users";
      if (this.isScimGroup) return "Users cannot be removed from SCIM groups.";
      if (this.isGoogleGroup)
        return "Users cannot be removed from the Google group.";
      return "Users cannot be removed from this group.";
    }
  },
  watch: {
    groupName: {
  ...
  methods: {
    callForTargetGroup() {
      getTargetGroup(this.resourceId).then((response) => {
        ...
        this.isScimGroup = !!data?.isScimGroup;
        this.isGoogleGroup = !!data?.isGoogleGroup;
    ...
    handleSearchChange(searchFilter = {}) {
      ...
      if (timeZoneIndex !== -1) {
        this.axiosPayload.filter.FilterGroups[1].FilterItems.splice(
          timeZoneIndex,
          1
        );
      }
    ...
    setStoredTableSettings(newColumns = []) {
      const renderedColumns = this?.$refs.refTargetGroupUsersTable
        ?.renderedColumns;
