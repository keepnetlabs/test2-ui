            ...
            generateLabels(chart = {}) {
              ...
              return data.datasets.map((item, index) => {
                const average = Math.round(
                  item.data.reduce((total, current) => total + current.y, 0) /
                    item.data.length
                );
                ...
                const percentage = average.toString().includes(".")
                  ? average.toFixed(2)
                  : average;
                const customSpacer =
        ...
        tooltips: {
          ...
          custom: function (tooltipModel) {
            ...
            if (tooltipModel.body && this?._chart?.data?.datasets) {
              ...
              tooltipFooter.style.opacity =
                dataIndex === 0 || comparatorValue === 0 ? 0 : 1;
