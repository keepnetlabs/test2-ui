    ...
    <v-card v-show="!loading" class="card">
      <v-list-item class="pl-2 pr-0 pb-8" v-if="title && title.icon">
        <div class="v-btn v-cart-icon-wrapper">
          <v-icon class="ml-2" color="blue" left medium>{{
            title.icon
          }}</v-icon>
        </div>
      ...
      <div class="table-wrapper">
            ...
            <slot name="addUsers">
              <v-tooltip bottom opacity="1">
                <template v-slot:activator="{ on }">
                  ...
                  >
                    <v-icon @click="addButtonFunction(addButton.action)"
                      >mdi-plus</v-icon
                    >
                  </v-btn>
                </template>
        ...
        <div
          class="selection-row"
          v-if="multipleSelection.length && tableData && tableData.length"
        >
          <v-checkbox
      ...
      <div
        ...
        v-if="pageSizes.length && tableData.length > 0 && isServerSide"
      >
        <el-pagination
...
export default {
        ...
        if (this.groupable && this.clusteredItems.length) {
          if (this.$refs && this.$refs.elTableRef) {
            ...
            for (const id of deletedIds) {
              this.clusteredItems = this.clusteredItems.filter(
                (item) => item[this.rowKey] !== id
              );
              const findedNewClusterItem = allItems.find(
                (i) => i[this.rowKey] === id
              );
              this.clusteredItems.push(findedNewClusterItem);
              this.$refs.elTableRef.toggleRowSelection(
                findedNewClusterItem,
                true
              );
            }
          }
  ...
  methods: {
    ...
    calculateAllSelected() {
      ...
      if (
        this.isSelectedAll &&
        this.multipleSelection.length === this.totalLength
      ) {
    ...
    renderFixedItems() {
      ...
      if (table) {
        ...
        const tableFixedRightItem = table.querySelector(
          ...
          const filterItems = this.columns.reduce((acc, filterItem) => {
            if (
              this.renderedColumns.find(
                (property) => property === filterItem.property
              )
            ) {
              acc.push({
                FieldName:
