        ...
        <v-form ref="form" lazy-validation>
          ...
          >
            <v-list-item-content>
              ...
              >
                Enter API Key {{ isIbmXForce ? "and Password" : "" }} generated by the
                provider
              </v-list-item-subtitle>
              <div
                v-if="!isCustomIntegration"
              ...
              >
                <div class="max-width__form">
                  <v-text-field
                    ...
                    :class="
                      item.status === 'failed' ? 'connection-error-state__border' : ''
                    "
                    :rules="[
                      apiKeyRules.required,
                      apiKeyRules.format,
                      apiKeyRules.maxLength,
                    ]"
              ...
              >
                <div
                  ...
                  @click="addApiKey"
                >
                  <v-icon color="#2196f3" style="cursor: pointer !important"
                    >mdi-plus</v-icon
                  >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      v-if="
                        isCustomIntegration && customIntegrationTestLoading === 'failed'
                      "
                      :id="`btn--integration-api-key-see-error-message-${index}`"
                      :style="isIbmXForce && { top: '49px !important' }"
                      class="connection-error-state"
                    ...
                    >
                      <v-icon
                        v-if="
                          isCustomIntegration &&
                          customIntegrationTestLoadingStatus === 'success'
                        "
                        :id="`btn--integration-api-key-check-${index}`"
                        class="ml-1"
                        color="#43a047"
                      ...
                      <div
                        v-if="
                          isCustomIntegration &&
                          customIntegrationTestLoadingStatus === 'failed'
                        "
                ...
                <v-tooltip bottom opacity="1">
                  <template v-slot:activator="{ on: tooltip }">
                    <v-icon
                      v-on="{ ...tooltip }"
                      :disabled="
                        !formValues.isSendUrl || !selectedIntegrationType.isSendUrl
                      "
          ...
          <v-list-item class="px-0 mt-6 mb-6">
            <v-list-item-content>
              ...
              <v-list-item-subtitle class="new-integration__api-key__subtitle">
                Uploading originally attached files to integrated services may lead
  ...
  data() {
    return {
      ...
      apiUrlRules: {
        ...
        maxLength: (v) =>
          Validations.maxLength(v, 5000, labels.getMaxLengthMessage("URL", 5000)),
      },
      apiKeyRules: {
        required: (v) => Validations.required(v),
        format: (v) => Validations.startsWithSpace(v),
        maxLength: (v) =>
          Validations.maxLength(v, 256, labels.getMaxLengthMessage("Api key", 256)),
      },
    };
  },
  computed: {
    ...
    isUSTA() {
      return this.selectedIntegrationType.name === INTEGRATION_TYPES.USTA;
    },
    isRoksit() {
  ...
  methods: {
    handleCacheDurationChange(val) {
      if (!val || /^\d{1,3}$/.test(val)) {
        ...
        this.$refs.refInputCacheDuration.initialValue = Number(
          this.formValues.cacheDuration
        );
        this.$refs.refInputCacheDuration.lazyValue = Number(
          this.formValues.cacheDuration
        );
      }
    },
    handleCacheQueryCountChange(val) {
      if (!val || /^\d{1,7}$/.test(val)) {
        ...
        this.$refs.refInputCacheQueryCount.initialValue = Number(
          this.formValues.cacheQueryCount
        );
        this.$refs.refInputCacheQueryCount.lazyValue = Number(
          this.formValues.cacheQueryCount
        );
      }
    },
      ...
      Promise.all(promises).then((response) => {
        ...
        this.uploadFileTypes = lookups.data.map((item) => {
          ...
          if (item.name === "Microsoft Office") {
            return {
              text:
                "Microsoft Office files (.doc, .docx, .xls, .xlsx, .ppt, .pptx, etc.)",
              value: item.name,
            };
          }
          if (item.name === "Other") {
    ...
    updateModels(response = {}) {
      ...
      if (
        ...
        response["data"].data.apiKeys = response["data"].data["apiCredentials"].map(
          (item) => {
            const obj = {
              value: item.apiKey,
              status: null,
    ...
    resetApiKeysAndCredentials() {
      if (!this.formValues.apiKeys) {
        this.$set(this.formValues, "apiKeys", [
          { value: "", status: null, resourceId: null },
        ]);
      }
    ...
    handleIntegrationTypeChange(val) {
      ...
      } else if (name === INTEGRATION_TYPES.USTA) {
        this.formValues.apiUrl =
          "https://usta.prodaft.com/api/threat-stream/v4/security-intelligence/ioc";
