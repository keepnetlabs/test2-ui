...
export default {
  ...
  methods: {
    ...
    getBadgeStatus(badge) {
      if (badge.earned) {
        const date = badge.earnedDate || badge.earnedOn || badge.createdAt
        if (!date) return ''
        return this.labels?.yourBadgesEarnedOn?.(this.formatBadgeEarnedDate(date)) ?? `Earned on ${this.formatBadgeEarnedDate(date)}`
      }
