...
export default {
      ...
      rules: [
        {
          ...
          textFieldValidations: {
            ...
            url: [
              ...
              (v) => Validations.required(v),
              (v) =>
                Validations.maxLength(v, 5000, labels.getMaxLengthMessage(labels.Domain, 5000)),
  ...
  methods: {
    ...
    checkIsQueryValid() {
      return this.query.children.every((child) => child.query.value && child.query.operand)
    },
    getErrorMessage() {
    ...
    handleCheckFiltersForDisableItems(logicalOperator = '') {
      ...
      if (this.query.children.filter((child) => child.query.operand === 'from').length > 1)
