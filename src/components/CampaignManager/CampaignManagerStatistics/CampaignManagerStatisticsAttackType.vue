...
export default {
  name: 'CampaignManagerStatisticsAttackType',
  components: {
    ExecutiveWidgetHeader,
    ExecutiveWidgetContainer,
    ExecutiveWidgetBody,
  ...
  props: {
    ...
    valueEnums: {
      type: Array,
      default: () => [labels.ClickOnly, labels.DataSubmitted, labels.MFA, labels.Attachment]
    },
    editMode: {
      type: Boolean,
      default: true
          ...
          labels: {
            ...
            generateLabels: (chart = {}) => {
              ...
              return data.datasets[0].data.map((d, index) => {
                ...
                const mfa = splittedLabel[0] === 'MFA'
                return {
                  text: Array.from(
                    (mfa ? label : '') +
                      label +
                      label +
                      label.substring(0, label.length / comparatorVal) +
                      d +
                      ' (%) '
                  )
                    .fill('')
                  ...
                  datasetIndex: index,
                  textParts,
                  customMarginLeft: label === 'Undetected' ? 12 : label === 'Simulation' ? 3 : 1
                }
              })
            }
          }
