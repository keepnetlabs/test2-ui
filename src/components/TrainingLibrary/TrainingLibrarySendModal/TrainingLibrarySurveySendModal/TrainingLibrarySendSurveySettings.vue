<template>
  <v-form ref="refForm" class="send-training-settings">
    ...
    <DeliveryMethod
      ...
      type="Survey"
      :sub-title="labels.SurveyDeliveryMethodSub"
    ...
    >
      <div class="campaign-manager-advanced-settings__other-settings-last">
        <v-checkbox
          ...
          id="input--send-survey-send-reminder"
          color="#2196f3"
          hide-details
        >
        </v-checkbox>
      ...
      <AlertBox
        ...
        icon-name="mdi-information"
        text="If this option is selected and there is no exam in the survey, the reminder will continue indefinitely."
        :slots="{ primaryAction: false, secondaryAction: false }"
      />
    </FormGroup>
    <FormGroup
    ...
    >
      <div class="d-flex align-center">
        <v-checkbox
          ...
          id="input--send-survey-award-certificate"
          hide-details
          color="#2196f3"
          label="Award certificate when a user completes the survey"
        >
        </v-checkbox>
        <KSelect
    ...
    >
      <div class="campaign-manager-advanced-settings__other-settings-last">
        <v-checkbox
          ...
          id="input--send-survey-auto-enroll"
          color="#2196f3"
          hide-details
        >
        </v-checkbox>
    ...
    <FormGroup :class="!isDeliveryMethodLMS ? 'mt-6' : ''" title="Mark as Test">
      <v-checkbox
        ...
        id="input--send-survey-marked-as-test"
        hide-details
        color="#2196f3"
...
export default {
  name: 'TrainingLibrarySendSurveySettings',
  components: {
    InputEntityName,
    AlertBox,
    InputContentLanguage,
  ...
  computed: {
    ...
    deliveryMethodText() {
      if (this.formData.deliveryMethod === DELIVERY_METHODS.LMS) {
        return "The survey will be distributed through the organization's LMS after it is downloaded and prepared. This process must be completed manually."
      } else if (this.formData.deliveryMethod === DELIVERY_METHODS.SMS) {
        return 'Selected users without a registered phone number will receive the survey via email.'
      } else if (this.formData.deliveryMethod === DELIVERY_METHODS.MICROSOFT_TEAMS) {
        return 'Selected users not found in Microsoft Teams will receive the survey via email.'
      }
      return ''
    },
    isDeliveryMethodSMS() {
