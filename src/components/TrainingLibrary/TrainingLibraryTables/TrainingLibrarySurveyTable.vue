...
export default {
  ...
  data() {
    return {
      CONSTANTS: {
        id: 'awareness-educator-training-library-survey-data-table'
      },
      tableOptions: {
        savedFiltersLocalStorageKey: DEFAULT_SEARCH_CONTAINER_KEYS.TRAINING_LIBRARY_SURVEY_TABLE,
        savedTableSettingsLocalStorageKey: TABLE_SETTINGS_KEYS.TRAINING_LIBRARY_SURVEY_TABLE,
        selectEvent: {
          clipboard: true,
          edit: false,
          delete: false,
        ...
        columns: [
          TRAINING_LIBRARY_COLUMNS.SURVEY_NAME,
    ...
    renderedColumns: {
      ...
      handler(renderedCols = []) {
        this.tableOptions.columns.forEach((col) => {
          if (col.property === PROPERTY_STORE.TRAINING_NAME) {
            col.show = true
            return
          }
