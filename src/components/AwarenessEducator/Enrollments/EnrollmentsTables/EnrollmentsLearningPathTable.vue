...
export default {
  name: 'EnrollmentsLearningPathTable',
  components: {
    EnrollmentsTableRowActions,
    DataTable
  },
  ...
  data() {
    return {
      ...
      axiosPayload: getDefaultAxiosPayload({
        enrollmentType: TRAINING_LIBRARY_PAYLOAD_TYPES.LEARNING_PATH
      }),
      tableData: [],
      serverSideProps: new ServerSideProps(),
      tableOptions: {
        savedFiltersLocalStorageKey: DEFAULT_SEARCH_CONTAINER_KEYS.ENROLLMENTS_LEARNING_PATH_LIST,
        savedTableSettingsLocalStorageKey: TABLE_SETTINGS_KEYS.ENROLLMENTS_LEARNING_PATH_LIST,
        selectEvent: {
          clipboard: true,
          edit: false,
          delete: false,
        ...
        iEmpty: {
          message: labels.EmptyEnrollmentLearningPath
        },
        addButton: {
          show: false
        },
    ...
    exportEnrollments(downloadTypes) {
      downloadTypes.exportTypes.forEach((item) => {
        ...
        AwarenessEducatorService.exportEnrollments(payload).then((response) => {
          ...
          link.download = `Learning-Path-List.${
