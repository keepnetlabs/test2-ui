  ...
  >
    <template #datatable-custom-column="{ scope, col }">
      ...
      <v-tooltip
        ...
        nudgeLeft="40"
        max-width="250"
        :disabled="getTooltipDisabilityStatus(scope.row)"
      >
...
export default {
    ...
    return {
      ...
      tableOptions: {
        savedFiltersLocalStorageKey:
          DEFAULT_SEARCH_CONTAINER_KEYS.TRAINING_REPORT_ENROLLMENT_EMAILS_TABLE,
      ...
      handler(val) {
        if (val) {
          const resendActionIndex = this.tableOptions.rowActions.findIndex(
            (action) =>
              action.name === "Resend Training" || action.name === "Resend Survey"
          );
  ...
  methods: {
    ...
    callForData() {
      ...
      AwarenessEducatorService.searchSendingReportEnrollmentEmails(
        this.axiosPayload,
        this.id
      )
        .then((response) => {
