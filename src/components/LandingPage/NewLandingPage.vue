  ...
  >
    <template #overlay-body>
            ...
            <div class="email-template-info">
              <v-list-item>
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle class="new-email-template__sub-title"
                    >Enter basic information about this landing page
              ...
              <v-form ref="refFormStep1" lazy-validation>
                ...
                <InputPhishingMethod
                  ...
                  :disabled="
                    selectedMethodText ? selectedMethodText !== 'MFA' : false
                  "
                />
                <form-group
                  title="Tags"
                  sub-title="Define tags for the template"
                >
                  <InputTag
            ...
            <div class="email-settings">
              <v-list-item>
                <v-list-item-content>
                  ...
                  >
                  <v-list-item-subtitle class="new-email-template__sub-title"
                    >Enter basic information about this email
                    template</v-list-item-subtitle
                  >
                </v-list-item-content>
              </v-list-item>
                ...
                <v-list-item-content>
                  ...
                  >
                    <div class="landing-page-tab-content">
                      ...
                      <div
                        v-if="isAIAllyOpen && !isGenerateWithAIDisabled"
                        class="mt-3"
                      >
                        <AIAllyMini
                          ...
                          :ai-assistant-remaining-right="
                            aiAssistantRemainingRights
                          "
                          @update:language-type-resource-id="
                            activeLanguage = $event
                          "
                          @update:template="handleAITemplateUpdate"
                          @on-assisted-by-ai-template="
                            handleAssistedByAITemplate
                          "
                          @update:ai-assistant-remaining-right="
                            aiAssistantRemainingRights = $event
                          "
                          @update:isEmailGenerating="
                            handleEmailGeneratingChange
                          "
                        />
                      </div>

                      <div
                        v-show="isPhishingLinkOpen && !isGenerateWithAIDisabled"
                        class="mt-3"
                      >
                        <InputPhishingLinkMini
                          ...
                          "
                          @invisible-captcha="
                            isInvisibleCaptchaDisabled = $event
                          "
                          @captcha-default-value="
                            formValues.isInvisibleCaptchaEnabled = $event
                          "
                          @captcha-change="
                            formValues.isInvisibleCaptchaEnabled = $event
                          "
                      ...
                      <el-tabs
                        ...
                        :class="{
                          'tabs-header-hidden':
                            isDefault || isGenerateWithAi || isAIAllyGenerating
                        }"
                      >
                        <el-tab-pane
                        ...
                        >
                          ...
                          <EmailTemplate
                            ...
                            :language-type-resource-id.sync="activeLanguage"
                            ...
                            :template.sync="
                              getSelectedLanguagePayload.landingPages[index]
                                .content
                            "
                            :method-type-id="formValues.methodTypeId"
                            :language-options="languageOptions"
                            ...
                            @on-custom-head-scripts-placement-change="
                              (value) =>
                                onCustomHeadScriptsPlacementChange(value, index)
                            "
                        ...
                        >
                          <template #label>
                            <div style="display: flex;">
                              ...
                              >
                                <v-icon class="mr-1" size="18" color="#2196f3"
      ...
      <SelectClickOnlyPageModal
        ...
        :languages="
          (scenarioDetailsLookup && scenarioDetailsLookup.languageTypes) ||
          (landingPageData && landingPageData.languageTypes) ||
          []
        "
        type="phishing"
...
export default {
  ...
  data() {
    return {
      ...
      formValues: {
        ...
        tags: [],
        landingPages: [
          { name: "landing-page", content: "", order: 1, prompt: "" }
        ],
        ...
        rules: [
          ...
          (v) =>
            Validations.maxLength(
              v,
              200,
              labels.getMaxLengthMessage(labels.TemplateName, 200)
            )
        ]
      },
  ...
  watch: {
    ...
    selectedLanguages(newVal) {
      ...
      if (!newVal.length) {
        ...
        // Aktif dil hala seçili mi?
        const isInSelected = newVal.find(
          (item) => item.value === this.activeLanguage
        );
        if (!isInSelected) {
          // Değilse yeni dil seç
          this.activeLanguage = newVal[0].value;
          // Eğer yeni dil languagesPayload'da yoksa ekle
  ...
  methods: {
    ...
    handleAITemplateUpdate(template) {
      ...
      if (this.formValues.landingPages[currentPageIndex]) {
        this.$set(
          this.formValues.landingPages[currentPageIndex],
          "content",
          template
        );
      }
      ...
      if (
        selectedLanguagePayload &&
        selectedLanguagePayload.landingPages[currentPageIndex]
      ) {
        this.$set(
          selectedLanguagePayload.landingPages[currentPageIndex],
          "content",
    ...
    handleEmailGeneratingChange(isGenerating) {
      ...
      if (
        this.$refs.refEmailTemplate &&
        this.$refs.refEmailTemplate[currentPageIndex]
      ) {
        this.$refs.refEmailTemplate[
          currentPageIndex
        ].isEmailGenerating = isGenerating;
      }
    },
    handleLinkChange() {
            ...
            if (data.isCompleted) {
              ...
              if (Array.isArray(data.translatedContents)) {
                ...
                const lastTranslation =
                  successLanguages[successLanguages.length - 1];
                if (lastTranslation) {
                  this.handleActiveLanguageChange(
                    lastTranslation.languageResourceId
                  );
                }
              }
            } else {
    ...
    showLocalizationErrorMessage(item) {
      const languageName =
        item.targetLanguage || item.languageResourceId || "Unknown";
    ...
    showLocalizationSuccessMessage(translatedContents) {
      ...
      const successItems = translatedContents.filter(
    ...
    handleAddBlankPage() {
      ...
      const maxOrder = Math.max(
        ...this.formValues.landingPages.map((p) => p.order || 0),
        0
      );
    ...
    getNewIndexForPageText() {
      ...
      let newPageIndex =
        this.editedLandingPages.length === 2
          ...
          : defaultIndex;
      const firstPageIndex = Number.parseInt(
        this?.formValues?.landingPages[0].name.split(" ")[1]
      );
    ...
    handleHTMLUploadChange(e) {
      ...
      reader.onload = function (e) {
        ...
        const maxOrder = Math.max(
          ...that.formValues.landingPages.map((p) => p.order || 0),
          0
        );
        ...
        // Tüm dillerin sayfa listelerine de aynı sayfayı ekle
        const lastPageIndex = that.formValues.landingPages.length - 1;
    ...
    nextStep() {
      ...
      if (this.$refs.refMakeAvailableFor) {
        this.$refs.refMakeAvailableFor.validateAvailableFor(
      ...
      if (this.$refs.refFormStep1.validate() && isValid) {
        ...
        const el = this.$refs.refFormStep1.$el.querySelector(
          ".v-messages__message"
        );
    ...
    getLanguageObject(languageTypeResourceId, languageTypeName = null) {
      ...
      for (const group of this.languageItems || []) {
        if (Array.isArray(group.children)) {
          const langObj = group.children.find(
            (item) => item.value === languageTypeResourceId
          );
          if (langObj) {
            return langObj;
          }
    ...
    setLanguageItems() {
      ...
      this.$nextTick(() => {
        ...
        const isEnglish = findedLanguage.text
          ?.toLowerCase()
          .includes("english");
        if (isEnglish) {
          const companyLanguagePayload = this.languagesPayload.find(
            (item) =>
              item.languageTypeResourceId === companyLanguageTypeResourceId
          );
        ...
        if (
          !isEnglish &&
          this.getSelectedLanguagePayload.landingPages &&
          !this.isEdit
        ) {
          this.isDefault = true;
  ...
  computed: {
    ...
    clickOnlyMethodText() {
      if (this.formValues.methodTypeId === "1") {
        return (
          this.landingPageData?.methodTypes?.find((item) => item.value === "1")
            ?.text || "Click Only"
        );
      }
      return "";
  ...
  created() {
    ...
    if (!this.isEdit)
      this.formValues.methodTypeId =
        this.landingPageData.methodTypes[0]?.value || "";
    if (this.landingPageData && this.selectedMethodText)
      this.formValues.methodTypeId = this.selectedMethodText.startsWith("Click")
        ? "1"
        : "2";
    ...
    if (this.isEdit) {
      getLandingPageTemplate(this.emailTemplateId).then((response) => {
        ...
        if (typeof data.phishingLink === "object") {
          phishingLink = {
            ...
            urlSchemaTypeId:
              ...
              "",
            pathTypeId:
              data?.phishingLink?.pathTypeId?.toString() ||
              data.pathTypeId.toString() ||
              "",
            extensionTypeId:
            ...
            parameterTypeId:
              ...
              "",
            domainRecordId:
              data?.phishingLink?.domainRecordId?.toString() ||
              data.domainRecordId.toString() ||
              ""
          };
        } else {
          phishingLink = {
        ...
        this.$nextTick(() => {
          if (
            this.$refs.refInputPhishingLinkMini &&
            phishingLink.domainRecordId
          ) {
            this.$refs.refInputPhishingLinkMini.checkSchemaTypes(
              phishingLink.domainRecordId
            );
          }
        });
        ...
        this.isInvisibleCaptchaDisabled = findedDomain
          ? !findedDomain?.extraDatas[1]?.value
          : false;
        if (this.isDuplicate)
        ...
        data.landingPages.forEach((page, index) => {
          const {
            customScripts,
            scriptsPlacement
          } = processTemplateWithCustomScripts(page.content);
