...
export default {
  ...
  methods: {
      ...
      this.editor.on("component:selected", () => {
        ...
        if (selected && selected.is("link")) {
          ...
          setTimeout(() => {
            ...
            let mergedTextsNames = this.urlMergedTexts.map(
              (item) => item.value
            );
            if (
              !mergedTextsNames.includes(
                selected.getTrait("href").props().value
              ) &&
              document.querySelector(
                "#gjsNewsletterModal > div.gjs-editor.gjs-one-bg.gjs-two-color > div.gjs-pn-panels > div.gjs-pn-panel.gjs-pn-views-container.gjs-one-bg.gjs-two-color > div:nth-child(3) > div:nth-child(1) > div.gjs-trt-traits.gjs-one-bg.gjs-two-color > div:nth-child(4) > div > div.gjs-field-wrp.gjs-field-wrp--select > div > div:nth-child(1) > select"
              )
          ...
          if (["Input", "Textarea", "Button", "Checkbox"].includes(name)) {
            const settingsTab = document.querySelector(
              '.gjs-pn-buttons span[title="Settings"]'
            );
            if (settingsTab) {
              settingsTab.click();
            }
    ...
    setPanels() {
      ...
      pn.getButton("options", "export-template").set(
        "className",
        "fa fa-import"
      );
      pn.getButton("options", "gjs-open-import-webpage").set(
        "className",
        "fa fa-code"
      );
      document.querySelector('span[title="Open Layer Manager"]').style.display =
        "none";
    ...
    setGrapesEditor() {
      ...
      const myNewComponentTypes = (editor) => {
        ...
        editor.DomComponents.addType("phishing-submit-button", {
          ...
          view: {
            ...
            handleTextChange(component, value) {
              if (
                component?.getEl()?.constructor?.name === "HTMLButtonElement"
              ) {
        ...
        editor.DomComponents.addType(
          "outlook-button",
          getComponentTypeDefaultParams(true, true)
        );
        editor.DomComponents.addType(
          "text",
          getComponentTypeDefaultParams(true, true)
        ...
        editor.DomComponents.addType(
          "label",
          getComponentTypeDefaultParams(false, true)
        );
        editor.DomComponents.addType("image", {
          ...
          model: {
            ...
            handleAttrChange(component, value) {
              try {
                if (
                  component?.getEl()?.parentElement?.constructor?.name ===
                  "HTMLAnchorElement"
                ) {
      ...
      this.editor.on("style:property:update", (styleChanges) => {
        if (
          ...
          styleChanges?.property?.attributes?.property !== "width"
        ) {
          return;
        }
        if (
        ...
        if (
          ...
          updatedComponent
            ?.getEl()
            ?.classList.contains("outlook-button-span-id")
        ) {
      ...
      this.editor.on("component:deselected", (updatedComponent) => {
        ...
        if (el?.id?.includes("outlook-button-href-id") && !el.innerText) {
          const commentElement = this.getCommentElementByComponent(
            updatedComponent
          );
      ...
      this.editor.on("component:remove", (component) => {
        if (component?.ccid?.includes("outlook-button-href-id")) {
          ...
          const outlookSpanItems = editor.DomComponents.getWrapper().find(
            ".outlook-button-span-id"
          );
          for (const element of outlookSpanItems) {
            if (!element?.toHTML?.()?.includes("outlook-button-href-id"))
              element.remove();
          }
        }
      });
    ...
    getCommentElementByComponent(component) {
      ...
      if (children) {
        return (
          children?.models?.find((el) => el?.attributes?.type === "comment") ||
          null
        );
      }
      return null;
    ...
    getGrapesWebModalDraw(html) {
      ...
      this.editor.on("load", () => {
        ...
        el = document.querySelector(
          ".gjs-blocks-cs .gjs-blocks-no-cat:last-child"
        );
        if (el) {
        ...
        document
          .querySelectorAll(".gjs-sm-sector-title")
        ...
        if (document.querySelector(".fa-code")) {
          document.querySelector(".fa-code").addEventListener("click", () => {
            ...
            btnCopyToClipboard.className =
              "ml-2 gjs-btn-prim gjs-btn-import mt-2";
            btnImp.onclick = () => {
    ...
    getGrapesEditorContent() {
      ...
      if (this.isEdit && this.editedCustomHeadScripts) {
        const scripts = htmlDOM.querySelectorAll(
          'script[data-custom-landing-page-script="true"]'
        );
      ...
      if (this.customHeadScripts && this.customHeadScripts.trim()) {
        const scriptsPlacement =
          this.customHeadScriptsPlacement === "body-end"
            ? "body-end"
            : "body-start";
        const createScript = (script) => {
          ...
          newScript.setAttribute(
            "data-custom-landing-page-script-position",
            scriptsPlacement
          );
      ...
      if (head) {
        ...
        if (htmlElement) {
          ...
          newHtmlDOM.innerHTML = html?.trim().startsWith("<body")
            ? html
            : htmlDOM.innerHTML;
          ...
          addedScripts.forEach((script) => {
            newHtmlDOM
              .querySelector("body")
              .insertAdjacentElement(insertPosition, script);
          });
