          ...
          >
            ...
            <template #template-header-right>
              <InputLanguagesSettings
                ...
                :is-template-type-selected="!!formValues.emailTemplateCategoryResourceId"
  ...
  methods: {
        ...
        .then((response) => {
          ...
          if (response?.data?.data?.languages && response?.data?.data?.languages.length) {
            response?.data?.data?.languages.forEach((item) => {
              ...
              if (item.languageTypeResourceId === this.formValues.languageTypeResourceId) {
                return
              }
              this.selectedLanguages.push({
    ...
    setLanguageItems() {
      ...
      if (
        ...
        if (categoryIndex !== -1 && this.categoryItems[categoryIndex].template) {
          this.formValues.template = this.categoryItems[categoryIndex].template
          // initialFormValues'a da set et
          this.initialFormValues.template = this.categoryItems[categoryIndex].template
        }
      }

      // Eğer selectedLanguages'da yoksa ekle ve handleSelectedLanguagesChange çağır
      if (!this.selectedLanguages.find((lang) => lang.value === companyLanguageTypeResourceId)) {
        this.selectedLanguages.push({
    ...
    submit() {
      ...
      if (refForm.validate() && isValid) {
        ...
        if (mainLanguage) {
          this.formValues.subject = mainLanguage.subject || this.formValues.subject || ''
          this.formValues.fromName = mainLanguage.fromName || this.formValues.fromName || ''
    ...
    handleSelectedLanguagesChange(languages) {
      this.languagesPayload = languages.map((language) => {
        ...
        return {
          ...
          template: (this.initialFormValues && this.initialFormValues.template) || '',
    ...
    askForNotificationTemplateTranslation(count = 0, maxCount = null, timeoutId = 0) {
        ...
        getNotificationTemplateTranslation()
          .then((response) => {
            ...
            successLanguages.forEach((item) => {
              ...
              languagePayload.fromName = item.fromName || languagePayload.fromName
