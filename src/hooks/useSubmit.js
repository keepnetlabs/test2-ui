;(function () {
  ...
  function getForms(selector = document, selection = 'form') {
    return selector.querySelectorAll(selection)
  }

  function setActiveForm(state) {
    const { forms } = state
    if (forms.length) {
      for (const form of forms) {
        const inputs = form.querySelectorAll('input')
        if (inputs.length) {
          updateState('activeForm', form)
          break
        }
      }
    }
  ...
  function setSubmitButton(state) {
    ...
    const submitButton =
      activeForm.querySelector('input[type="submit"]') ||
      activeForm.querySelector('button[type="submit"]')
    if (!submitButton) return
    updateState('submitButton', submitButton)
  }

  function handleSubmit(formData) {
    try {
      fetch(`${window.location.origin}/ps-tracking/submit`, {
        method: 'POST', // or 'PUT'
        headers: {
  ...
  function setEventListeners(state) {
    ...
    activeForm.addEventListener('submit', function (e) {
      //adding for not refreshing
      e.preventDefault()
      const formData = {}
      const inputs = e.target.querySelectorAll('input')
      let complexity = 0
      inputs.forEach((input, index) => {
        const key = input['name'] || `input-${input.type}-${index}`
        if (input.type === 'submit' || input.type === 'reset') return
          ...
          complexity = calculatePasswordComplexity(input.value)
          const { value } = input
          let chars = ''
          for (let i = 0; i < value.length - 1; i++) {
            chars += '*'
          }
          formData[key] = `${value.substring(0, 1)}${chars}`
        } else {
          formData[key] = input.value
        }
      })
      ...
      const payload = {
        ...
        trackingData,
        passwordComplexityType: complexity
      }
  ...
  function calculatePasswordComplexity(pwd) {
