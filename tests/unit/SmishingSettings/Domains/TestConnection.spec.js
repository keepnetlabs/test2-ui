...
import TestConnection from '@/components/SmishingSettings/Domains/TestConnection.vue'
...
describe('SmishingSettings Domains TestConnection.vue', () => {
  ...
  it('testConnection handles success response', async () => {
    ...
    expect(SmishingService.testDomainConnection).toHaveBeenCalledWith({
      dnsServiceProviderId: 'dns-1',
      zoneId: 'zone-1'
    })
  ...
  it('testConnection handles error with validationMessages', async () => {
    SmishingService.testDomainConnection.mockRejectedValueOnce({
      response: {
        data: {
