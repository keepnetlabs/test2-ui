...
import TestConnection from '@/components/SmishingSettings/DnsServices/TestConnection.vue'
...
describe('SmishingSettings DNS TestConnection.vue', () => {
  ...
  it('testConnection returns existing success when already tested', () => {
    ...
    expect(SmishingService.testConnection).not.toHaveBeenCalled()
    expect(wrapper.emitted('testConnectionValues')[0]).toEqual([true, true])
  })
  ...
  it('testConnection handles api success', async () => {
    ...
    expect(SmishingService.testConnection).toHaveBeenCalledWith(
      {
        dnsServiceProviderTypeId: 1,
        username: 'user@test.com',
        password: 'secret',
  ...
  it('testConnection handles error and uses validation message', async () => {
    SmishingService.testConnection.mockRejectedValueOnce({
      response: {
        data: {
