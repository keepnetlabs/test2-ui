...
describe('TrainingLibraryScreensaverTable.vue (extra)', () => {
  it('selectedTrainingContent watcher swaps empty state between favourites and default', () => {
    const ctx = {
      tableOptions: {
        iEmpty: { message: labels.EmptyScreensaver, btn: labels.CreateNewScreensaver }
      },
      $set: (obj, key, value) => {
        obj[key] = value
      }
    ...
    TrainingLibraryScreensaverTable.watch.selectedTrainingContent.handler.call(
      ctx,
    ...
    expect(ctx.tableOptions.iEmpty.message).toBe(labels.EmptyScreensaver)
    expect(ctx.tableOptions.iEmpty.btn).toBe(labels.CreateNewScreensaver)
  })

  it('renderedColumns watcher keeps screensaver name visible', () => {
    const ctx = {
      tableOptions: {
        columns: [
          { property: PROPERTY_STORE.SCREENSAVER_NAME, show: false },
          { property: PROPERTY_STORE.LANGUAGE, show: false }
        ]
      }
    }

    TrainingLibraryScreensaverTable.watch.renderedColumns.handler.call(ctx, [PROPERTY_STORE.LANGUAGE])
  ...
  it('fixed-column watchers sync both column metadata and table refs', () => {
    ...
    TrainingLibraryScreensaverTable.watch.firstColFixed.call(ctx)
    TrainingLibraryScreensaverTable.watch.lastColFixed.call(ctx)
  ...
  it('serverSideSizeChanged resets page numbers and refetches data', () => {
    ...
    const ctx = {
      axiosPayload: { pageSize: 5, pageNumber: 2 },
      serverSideProps: { pageSize: 5, pageNumber: 2 },
      callForData
    }

    TrainingLibraryScreensaverTable.methods.serverSideSizeChanged.call(ctx, 30)

    expect(ctx.axiosPayload.pageSize).toBe(30)
    expect(ctx.serverSideProps.pageSize).toBe(30)
