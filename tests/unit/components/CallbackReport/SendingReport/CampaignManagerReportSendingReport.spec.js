...
describe('CallbackReport CampaignManagerReportSendingReport.vue', () => {
  ...
  it('getLastSendingStatusItems returns userStatuses from formDetails', () => {
    const wrapper = mountComponent({
      formDetails: { userStatuses: [{ id: 1, name: 'Delivered' }] }
    })
    expect(wrapper.vm.getLastSendingStatusItems).toEqual([{ id: 1, name: 'Delivered' }])
  })

  it('getLastSendingStatusItems returns empty array when formDetails missing', () => {
    const wrapper = mountComponent({ formDetails: {} })
