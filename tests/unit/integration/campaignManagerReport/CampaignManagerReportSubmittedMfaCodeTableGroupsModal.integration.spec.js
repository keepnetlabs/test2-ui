/**
 * Submitted MFA Code tablo: gruplar sütunu → `CommonReportViewTargetGroupsModal`.
 */
...
const groupsDataTableStub = {
  ...
  template:
    '<div class="dt-mfa-groups"><slot name="datatable-custom-column" :scope="{ row: table[0] }" :col="groupsColumn" /></div>'
}
...
describe('Campaign Manager Report Submitted MFA Code table ↔ groups modal (integration)', () => {
  let vuetify
  ...
  it('opens groups modal and clears on AppDialog close', async () => {
    const wrapper = mount(CampaignManagerReportSubmittedMfaCodeTable, {
      ...
      propsData: { id: 'mfa1', instanceGroup: 'ig-mfa' },
      data() {
        return {
          tableData: [{ firstName: 'M', lastName: 'FA', targetGroups: 'A, B' }]
        }
      },
      methods: {
