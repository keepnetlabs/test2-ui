/**
 * Phishing Reporter tablo: gruplar sütunu → `CommonReportViewTargetGroupsModal`.
 */
...
const groupsDataTableStub = {
  ...
  template:
    '<div class="dt-phish-groups"><slot name="datatable-custom-column" :scope="{ row: table[0] }" :col="groupsColumn" /></div>'
}
...
describe('Campaign Manager Report Phishing Report table ↔ groups modal (integration)', () => {
  let vuetify
  ...
  it('opens groups modal and clears on close', async () => {
    const wrapper = mount(CampaignManagerReportPhishingReportTable, {
      ...
      propsData: { id: 'pr1', instanceGroup: 'ig-pr' },
      data() {
        return {
          tableData: [{ firstName: 'P', lastName: 'R', targetGroups: ['X', 'Y'] }]
        }
      },
      methods: {
    ...
    expect(wrapper.vm.selectedGroups).toEqual([{ name: 'X' }, { name: 'Y' }])
